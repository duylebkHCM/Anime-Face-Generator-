...
class Critic(nn.Module):
    ...
    def __init__(self, input_dim=1, hidden_dim=32):
        ...
        self.crit = nn.Sequential(
            self.make_crit_block(input_dim, hidden_dim),
...
class WGAN_GP(object):
    def __init__(self, args):
        ...
        self.C = Critic(input_dim=data.shape[1], hidden_dim=self.hdd)
        self.G_optimizer = optim.Adam(self.G.parameters(), lr=args.lrG, betas=(args.beta1, args.beta2))
        self.C_optimizer = optim.Adam(self.C.parameters(), lr=args.lrD, betas=(args.beta1, args.beta2))
    ...
    def gradient_penalty(self, gradient):
        ...
        penalty = ((gradient_norm - 1).mean())**2
        return penalty
            ...
            with tqdm(self.data_loader, unit="batch") as tepoch:
                for iter, (x_, _) in enumerate(tepoch):
                    ...
                    for _ in range(self.critic_repeats):
                        ...
                        z1_ = torch.randn((self.batch_size, self.z_dim))
                        if self.gpu_mode:
                            x_, z1_ = x_.cuda(), z1_.cuda()

                        G_ = self.G(z1_)
                        C_real = self.C(x_)
                        C_fake = self.C(G_.detach())

                        epsilon = torch.rand(self.batch_size, 1, 1, 1, requires_grad=True)
                        if self.gpu_mode:
                            epsilon = epsilon.cuda()

                        gradient = self.get_gradient(self.C, x_, G_.detach(), epsilon)
                        gp = self.gradient_penalty(gradient)
                        crit_loss = self.get_crit_loss(C_fake, C_real, gp, self.c_lambda)
                        # Keep track of the average critic loss in this batch
                        mean_iteration_critic_loss += crit_loss.item() / self.critic_repeats

                        crit_loss.backward(retain_graph=True)
                        self.C_optimizer.step()

                    self.train_hist['C_loss'].append(mean_iteration_critic_loss)
                    ...
                    G_ = self.G(z_)
                    C_fake = self.C(G_)
                    gen_loss = self.get_gen_loss(C_fake)
                    gen_loss.backward()
                    self.G_optimizer.step()

                    self.train_hist['G_loss'].append(gen_loss.item())

                    tepoch.set_postfix(C_loss= mean_iteration_critic_loss,  G_loss=gen_loss.item())
